...
const DAILY_EXTERNAL_TEST_LIMIT = 10;
...
function getExternalTestUsageState() {
    const history = loadUsageHistory(getScopedStorageKey(DAILY_EXTERNAL_TEST_STORAGE_KEY), DAILY_EXTERNAL_TEST_WINDOW_MS);
    const used = history.length;
    const remaining = Math.max(0, DAILY_EXTERNAL_TEST_LIMIT - used);
    return { used, remaining, limit: DAILY_EXTERNAL_TEST_LIMIT };
}

function recordExternalTestCompletion() {
    const storageKey = getScopedStorageKey(DAILY_EXTERNAL_TEST_STORAGE_KEY);
    const history = loadUsageHistory(storageKey, DAILY_EXTERNAL_TEST_WINDOW_MS);
...
function formatDailyTestLimitText(remaining, limit) {
    if (currentLang === 'kk') {
        return `24 сағатта басқа қолданушылардың ${limit} тестіне дейін өтуге болады. Қалғаны: ${remaining}/${limit}.`;
    }
    if (currentLang === 'ru') {
        return `За 24 часа можно пройти до ${limit} тестов от других пользователей. Осталось: ${remaining}/${limit}.`;
    }
    return `You can take up to ${limit} tests from others in 24 hours. Remaining: ${remaining}/${limit}.`;
}

function formatDailyLimitReachedText() {
    if (currentLang === 'kk') {
        return '24 сағатқа арналған тест лимиті аяқталды.';
    }
    if (currentLang === 'ru') {
        return 'Лимит тестов от других пользователей на 24 часа исчерпан.';
    }
    return 'Daily limit for other users\' tests reached.';
}
...
function updateDailyTestLimitUI() {
    ...
    if (!infoEl) return;
    const { remaining, limit } = getExternalTestUsageState();
    infoEl.textContent = formatDailyTestLimitText(remaining, limit);
}
...
const i18n = {
    kk: {
        ...
        faq1a: 'õzger - ЕНТ-ға дайындық платформасы. AI Мұғалім көмегімен материалды үйреніп, тесттер өтуге болады.',
        ...
        inDevelopment: 'Дайындалуда',
        ratingInDevelopment: 'Рейтинг бөлімі дайындалуда',
    ...
    ru: {
        ...
        faq1a: 'õzger - платформа подготовки к ЕНТ. С помощью AI Учителя можно изучать материал и проходить тесты.',
        ...
        ratingInDevelopment: 'Раздел рейтинга в разработке',
    ...
    en: {
        ...
        faq1q: 'What is õzger?',
        faq1a: 'õzger is an ENT exam preparation platform. Use AI Teacher to learn material and take tests.',
        ...
        ratingInDevelopment: 'Rating section is under development',
...
async function initializeSocket() {
    ...
    try {
        ...
        socket.on('connect', () => {
            ...
            showToast('Онлайн режимге қосылдыңыз', 'success');
        });
        ...
        socket.on('rating_changed', (data) => {
            if (isInRatingPage) {
                updateRating();
                showToast(`${data.username} рейтингті жаңартты!`, 'success');
            }
        });
...
function updateOnlineUsersUI() {
    ...
    if (isInRatingPage) {
        ...
        ratingItems.forEach(item => {
            const userName = item.querySelector('.rating-name')?.textContent;
            ...
            if (userName) {
                ...
                if (isOnline) {
                    if (!onlineIndicator) {
                        ...
                        item.querySelector('.rating-user').appendChild(indicator);
                    }
                } else {
                    if (onlineIndicator) {
...
function sendRatingUpdate(score, subject) {
    if (socket && socket.connected) {
        socket.emit('rating_update', { score, subject });
    }
}


function joinRatingRoom() {
    if (socket && socket.connected) {
        socket.emit('join_rating');
        isInRatingPage = true;
    }
}

function leaveRatingRoom() {
    if (socket && socket.connected) {
        socket.emit('leave_rating');
        isInRatingPage = false;
    }
}
...
async function showRating() {
    console.log('showRating called - function is working!');
    hideAllPages();
    const ratingPage = document.getElementById('ratingPage');
    if (ratingPage) {
        ratingPage.classList.remove('hidden');
        console.log('ratingPage shown');
    } else {
        console.error('ratingPage element not found');
    }

    
    await new Promise(resolve => setTimeout(resolve, 100));

    console.log('Calling updateRating...');
    try {
        await updateRating();
        console.log('updateRating completed successfully');
    } catch (error) {
    ...
    try {
        ...
        console.error('Failed to join rating room:', error);
    }
}
...
async function toggleFavorite(materialId) {
    ...
    try {
        ...
        const isFavorited = result.favorite !== undefined;
        sendUserActivity('favorite_added', {
...
function initializeRatingFilters() {
    console.log('🎛️ Initializing rating filters...');

    const ratingTypeSelect = document.getElementById('ratingType');
    const ratingSubjectSelect = document.getElementById('ratingSubject');
    const ratingScopeSelect = document.getElementById('ratingScope');

    
    if (ratingSubjectSelect) {
        ratingSubjectSelect.classList.add('hidden');
    }

    if (ratingTypeSelect) {
        ratingTypeSelect.addEventListener('change', function() {
            const selectedType = this.value;
            console.log('📝 Rating type changed to:', selectedType);

            
            if (ratingSubjectSelect) {
                if (selectedType === 'subject') {
                    ratingSubjectSelect.classList.remove('hidden');
                    console.log('👁️ Subject filter shown');
                } else {
                    ratingSubjectSelect.classList.add('hidden');
                    console.log('🙈 Subject filter hidden');
                }
            }
            setTimeout(() => updateRating(), 10);
        });
    }

    
    if (ratingSubjectSelect) {
        ratingSubjectSelect.addEventListener('change', function() {
            console.log('📚 Subject filter changed to:', this.value);
            setTimeout(() => updateRating(), 10);
        });
    }

    if (ratingScopeSelect) {
        ratingScopeSelect.addEventListener('change', function() {
            console.log('🏫 Scope filter changed to:', this.value);
            
            setTimeout(() => updateRating(), 10);
        });
    }

    console.log('✅ Rating filters initialized');
}
...
async function updateRating() {
    ...
    if (!ratingList) {
        console.error('❌ ratingList element not found - DOM not ready');
        return;
    }

    
    const ratingScope = document.getElementById('ratingScope');
    if (!ratingScope) {
        console.error('❌ ratingScope filter not found');
        return;
    }

    
    const ratingType = document.getElementById('ratingType')?.value || 'ent';
    const ratingSubject = document.getElementById('ratingSubject')?.value || '';
    const ratingScopeValue = ratingScope.value || 'all';

    console.log('✅ DOM ready, filters:', { ratingType, ratingSubject, ratingScope: ratingScopeValue });

    console.log('ratingList found, BACKEND_URL:', BACKEND_URL);
    ...
    setTimeout(async () => {
        try {
            ...
            const ratingType = document.getElementById('ratingType')?.value || 'ent';
            const ratingSubject = document.getElementById('ratingSubject')?.value || '';
            const ratingScope = ratingScopeValue;

            
            const params = new URLSearchParams();
            params.append('type', ratingType);
            if (ratingSubject) params.append('subject', ratingSubject);
            params.append('scope', ratingScopeValue);

            const url = `${BACKEND_URL}/api/rating?${params.toString()}`;
            console.log('Fetching ratings from:', url, 'with filters:', { ratingType, ratingSubject, ratingScope });
            ...
            if (!Array.isArray(ratings) || ratings.length === 0) {
                throw new Error('No rating data received');
            }

    console.log(`✅ Loaded ${ratings.length} ratings in ${responseTime.toFixed(1)}ms`);
        displayRatings(ratings);

        
        const successDiv = document.createElement('div');
        successDiv.style.cssText = 'background: #e8f5e8; color: #2e7d32; padding: 8px; margin: 10px 0; border-radius: 4px; font-size: 12px; font-weight: 500;';
        successDiv.innerHTML = `✅ Рейтинг загружен (${responseTime.toFixed(0)}ms) - ${ratings.length} участников`;
        ratingList.insertBefore(successDiv, ratingList.firstChild);
            ...
            errorDiv.style.cssText = 'background: #ffebee; color: #c62828; padding: 8px; margin: 10px 0; border-radius: 4px; font-size: 12px;';
            errorDiv.innerHTML = `⚠️ ${error.message}`;
            ratingList.insertBefore(errorDiv, ratingList.firstChild);
...
function showFallbackRating() {
    console.log('Showing fallback rating data...');

    const fallbackRatings = [
        { id: 'fallback-1', username: 'Айжан С.', school: 'Dostyq School', class: '11А', score: 125, totalTests: 15, averageScore: 115.5 },
        { id: 'fallback-2', username: 'Данияр Н.', school: 'NIS', class: '11Б', score: 118, totalTests: 12, averageScore: 108.25 },
        { id: 'fallback-3', username: 'Мадияр А.', school: 'БИЛ', class: '11А', score: 115, totalTests: 18, averageScore: 112.75 },
        { id: 'fallback-4', username: 'Асем К.', school: 'Dostyq School', class: '11В', score: 112, totalTests: 10, averageScore: 105.0 },
        { id: 'fallback-5', username: 'Жансая М.', school: 'NIS', class: '11А', score: 108, totalTests: 14, averageScore: 102.33 }
    ];

    displayRatings(fallbackRatings);
}
...
function displayRatings(ratings) {
    ...
    ratings.forEach((user, index) => {
        ...
        const isCurrentUser = currentUser && (
            user.username === currentUser.user_metadata?.username ||
            user.id === currentUser.id
        );

        ratingItem.className = `rating-item ${isCurrentUser ? 'self' : ''}`;
        ...
        nameDiv.textContent = user.username + (isCurrentUser ? ' (Вы)' : '');
...
function getFaqItems() {
    ...
    const ruItems = [
        ...
        { q: "\u041f\u043e\u0447\u0435\u043c\u0443 \u0435\u0441\u0442\u044c \u043b\u0438\u043c\u0438\u0442 \u043d\u0430 \u0442\u0435\u0441\u0442\u044b \u0434\u0440\u0443\u0433\u0438\u0445 \u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u0435\u0439?", a: "\u041b\u0438\u043c\u0438\u0442 \u043d\u0443\u0436\u0435\u043d \u0434\u043b\u044f \u0441\u0442\u0430\u0431\u0438\u043b\u044c\u043d\u043e\u0441\u0442\u0438 \u0438 \u0441\u043f\u0440\u0430\u0432\u0435\u0434\u043b\u0438\u0432\u043e\u0441\u0442\u0438. \u041e\u043d \u043e\u0431\u043d\u043e\u0432\u043b\u044f\u0435\u0442\u0441\u044f \u043a\u0430\u0436\u0434\u044b\u0435 24 \u0447\u0430\u0441\u0430." },
    ...
    const enItems = [
        ...
        { q: "Why is there a limit on other users tests?", a: "The limit keeps the system fair and stable. It resets every 24 hours." },
    ...
    const items = { kk: ruItems, ru: ruItems, en: enItems };
...
function getGuideItems() {
    ...
    const ruItems = [
        ...
        { q: "\u041b\u0438\u043c\u0438\u0442\u044b", a: "\u0415\u0441\u0442\u044c \u0441\u0443\u0442\u043e\u0447\u043d\u044b\u0439 \u043b\u0438\u043c\u0438\u0442 \u043d\u0430 \u0442\u0435\u0441\u0442\u044b \u0434\u0440\u0443\u0433\u0438\u0445 \u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u0435\u0439. `dailyTestLimitInfo` \u043f\u043e\u043a\u0430\u0437\u044b\u0432\u0430\u0435\u0442 \u043e\u0441\u0442\u0430\u0442\u043e\u043a." }
    ];
    const enItems = [
        ...
        { q: "Limits", a: "There is a daily limit for other users tests. `dailyTestLimitInfo` shows the remaining count." }
    ];
    const items = { kk: ruItems, ru: ruItems, en: enItems };
    return items[lang] || items.kk;
}
                ...
                <form class="auth-form form-step" id="authForm">
                    <div class="form-group">
                        ...
                        <select class="form-input form-select" id="regSchool">
                            <option value="none" ${!regData.school || regData.school === 'none' ? 'selected' : ''}>none</option>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select class="form-input form-select" id="regClassNumber">
                                <option value="none" ${!regData.classNumber || regData.classNumber === 'none' ? 'selected' : ''}>none</option>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-input form-select" id="regClassLetter">
                                <option value="none" ${!regData.classLetter || regData.classLetter === 'none' ? 'selected' : ''}>none</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-input form-select" id="regSubjectCombination">
                            <option value="none" ${!regData.subjectCombination || regData.subjectCombination === 'none' ? 'selected' : ''}>none</option>
...
function showCompletion() {
    ...
    sendRatingUpdate(totalPercent, currentSubject);
...
async function loadTestsFromSupabase() {
    if (!supabaseClient) {
        console.log('loadTestsFromSupabase: No supabase client');
        return [];
    }
    ...
    if (!userId) {
        try {
            ...
            console.log('Could not get session for user ID');
        }
    }
    
    console.log('Loading tests. Current user ID:', userId);
    
    try {
        console.log('Querying Supabase tests table...');
        
        
        const supabaseUrl = supabaseClient.supabaseUrl;
        const supabaseKey = supabaseClient.supabaseKey;
        
        const response = await fetch(`${supabaseUrl}/rest/v1/tests?select=*&order=created_at.desc`, {
            headers: {
                'apikey': supabaseKey,
                'Authorization': `Bearer ${supabaseKey}`,
                'Content-Type': 'application/json'
        ...
        if (!response.ok) {
            ...
            console.error('Error loading tests from Supabase:', response.status, errorText);
            return [];
        }
        
        const data = await response.json();
        
        console.log('Raw tests from Supabase:', data?.length);
        if (data && data.length > 0) {
            console.log('First test:', data[0]);
        } else {
            console.log('No tests returned from Supabase.');
        }
        ...
        supabaseTests = (data || []).map(t => {
            ...
            try {
                ...
                console.error('Error parsing questions for test:', t.id, e);
                questionsRaw = [];
            }
        ...
        console.log('Processed tests:', supabaseTests.length);
        console.log('Own tests count:', supabaseTests.filter(t => t.isOwn).length);
        return supabaseTests;
    } catch (error) {
        console.error('Error loading tests (catch):', error);
        return [];
    }
}
...
async function renderLibrary() {
    ...
    console.log('renderLibrary: supabaseClient exists?', !!supabaseClient);
    if (supabaseClient) {
        try {
            console.log('renderLibrary: calling loadTestsFromSupabase...');
            await loadTestsFromSupabase();
            console.log('renderLibrary: loadTestsFromSupabase completed, supabaseTests.length =', supabaseTests.length);
        } catch (e) {
            console.error('Error loading tests:', e);
        }
    } else {
        console.warn('supabaseClient is null - tests cannot be loaded');
    }
    ...
    console.log('Rendering library. Total tests loaded:', supabaseTests.length);
    console.log('Current tab:', currentLibraryTab);
    ...
    if (currentLibraryTab === 'my') {
        ...
        console.log('My tests:', materials.length);
        ...
        console.log('All visible tests:', materials.length);
    }
    
    
    if (currentSubjectFilter && currentSubjectFilter !== 'all') {
        console.log('Filtering by subject:', currentSubjectFilter);
        const beforeFilter = materials.length;
        materials = materials.filter(m => {
            const match = m.category === currentSubjectFilter || m.subject === currentSubjectFilter;
            if (!match) {
                console.log(`Test "${m.title}" filtered out: category="${m.category}", subject="${m.subject}"`);
            }
            return match;
        });
        console.log(`Subject filter: ${beforeFilter} -> ${materials.length} tests`);
    }
    ...
    console.log('Materials to render:', materials.length, materials.map(m => ({ id: m.id, title: m.title, isOwn: m.isOwn, subject: m.subject })));
...
function checkGuess() {
    ...
    const correctNames = Object.values(currentFigure.name).map(n => n.toLowerCase());
    const isCorrect = correctNames.some(name => {
        
        const nameParts = name.split(' ');
        return name === userAnswer || 
               nameParts.some(part => part === userAnswer) ||
               userAnswer.includes(nameParts[0]) ||
               name.includes(userAnswer);
    });
...
function confirmExitEditor() {
    ...
    const hasContent = editorQuestions.some(q => q.text || q.options.some(o => o)) ||
...
function normalizeQuestionsPayload(payload) {
    if (!payload) return { items: [], order: 'static' };
    if (Array.isArray(payload)) return { items: payload, order: 'static' };
    if (typeof payload === 'object' && Array.isArray(payload.items)) {
        return { items: payload.items, order: payload.order === 'random' ? 'random' : 'static' };
    }
    return { items: [], order: 'static' };
}
...
function deleteCurrentQuestion() {
    ...
    editorQuestions.splice(currentQuestionIndex, 1);
...
function addQuestionImage() {
    ...
    showToast('Image support removed', 'info');
}
...
async function renderClassmates() {
    ...
    console.log('=== Classmates Debug ===');
    console.log('Profile:', profile);
    console.log('userCity:', userCity, 'userSchool:', userSchool, 'userClass:', userClass);
    ...
    try {
        if (supabaseClient && userCity && userSchool && userClass) {
            ...
            console.log('Fetching classmates from:', url);
            ...
            if (response.ok) {
                ...
                const errorText = await response.text();
                console.error('Error fetching classmates:', response.status, errorText);
            }
        } else {
    ...
    console.log('Classmates after fetch:', classmates.length);
...
function isExternalTestAllowed() {
    const { remaining } = getExternalTestUsageState();
    return remaining > 0;
}

function canStartExternalTest(testData) {
    if (isOwnTest(testData)) return true;
    if (!isExternalTestAllowed()) {
        showToast(formatDailyLimitReachedText(), 'warning');
        updateDailyTestLimitUI();
        return false;
    }
    return true;
...
const AITeacherAPI = {
    async uploadMaterial(material) {
        ...
        const response = await fetch(`${AI_TEACHER_API_URL}/upload`, {
            method: 'POST',
            body: formData
        });
        ...
        if (!response.ok) {
            ...
            throw new Error(error.error || t('uploadError') || 'Upload error');
        }
        return response.json();
    },
    
    async generateLearn(materialId, historyMode = false) {
        const response = await fetch(`${AI_TEACHER_API_URL}/generate/learn`, {
            method: 'POST',
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.error || t('generationError') || 'Generation error');
        }
        return response.json();
    },
    
    async generatePractice(materialId, count, excludeQuestions = []) {
        const response = await fetch(`${AI_TEACHER_API_URL}/generate/practice`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async generateRealTest(materialId, count) {
        const response = await fetch(`${AI_TEACHER_API_URL}/generate/realtest`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    async healthCheck() {
        try {
            const response = await fetch(`${AI_TEACHER_API_URL}/health`);
            return response.ok;
        } catch {
            return false;
        }
...
function openAITeacherModal(mode) {
    if (!AI_TEACHER_API_URL) {
...
function renderAILearnPlan() {
    ...
    AITeacher.plan.forEach((section, i) => {
        html += `
            <button class="ai-plan-item ${i === AITeacher.currentSection ? 'active' : ''} ${i < AITeacher.currentSection ? 'completed' : ''}" 
...
function showAILearnSection() {
    ...
    const percent = ((AITeacher.currentSection + 1) / AITeacher.plan.length) * 100;
    if (progressFill) progressFill.style.width = `${percent}%`;
    if (progressText) progressText.textContent = `${AITeacher.currentSection + 1}/${AITeacher.plan.length}`;
    ...
    AITeacher.sectionQuestions = section.questions || [];
    AITeacher.questionQueue = [...AITeacher.sectionQuestions];
